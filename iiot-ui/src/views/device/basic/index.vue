    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="设备编号【设备ID】" prop="deviceSn">
          <el-input v-model="form.deviceSn" placeholder="请输入设备编号【设备ID】" />
        </el-form-item>
        ...
        <el-form-item label="设备最后活动时间" prop="deviceLastActiveTime">
          <el-date-picker clearable
            v-model="form.deviceLastActiveTime"
            type="date"
...
export default {
  ...
  methods: {
    ...
    handleUpdate(row) {
      this.reset();
      const deviceBasicId = row.deviceBasicId || this.ids
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.deviceBasicId != null) {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const deviceBasicIds = row.deviceBasicId || this.ids;
      this.$modal.confirm('是否确认删除设备基础编号为"' + deviceBasicIds + '"的数据项？').then(function() {
        return delBasic(deviceBasicIds);
      }).then(() => {
        this.getList();
